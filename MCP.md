...
- AtlasNode uses local-path-only `BAAI/bge-m3` embeddings and selectively reindexes documents when content or embedding configuration changes.
...
- Required local model path for BGE-M3: `ATLASNODE_EMBEDDING_MODEL_PATH`.
...
- For security, BGE-M3 only loads from the local path above, with remote model code execution disabled.
